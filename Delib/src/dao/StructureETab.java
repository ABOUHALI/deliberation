...
public class StructureETab  {
		...
		try {
			...
			while (rs.next()) {
				Semestre s = new Semestre(rs.getInt("id_semestre"), rs.getString("libelle_semestre"),
						rs.getInt("fid_etap"),rs.getInt("note_validation"));
				ss.add(s);
			}
		} catch (Exception e) {
			// TODO: handle exception
	...
	public Module getIdModule(int id) {
		Connection conn=SingletonConnection.getConnection();
		PreparedStatement ps;
		Module s=new Module();
		try {
			ps=conn.prepareStatement("select *from module where id_module=?");
			ps.setInt(1, id);
			ResultSet rs=ps.executeQuery();
			while(rs.next()) {
				s.setIDModule(id);
				s.setCoeff(rs.getInt("coefficient"));
				s.setLabelleMod(rs.getString("libelle_module"));
				s.setNbr_elt(rs.getInt("nbr_elt"));
				s.setNote_valid(rs.getInt("note_valid"));
				s.setIDSemestre(rs.getInt("fid_semestre"));
			}
			ps.close();
			conn.close();
	...
	public int IDanneACAD(int y) {
		Connection conn = SingletonConnection.getConnection();
		PreparedStatement ps;
		int id = 0;
		try {
			ps = conn.prepareStatement("select id_anneAcad from anneacad where anneAcad=?");
			ps.setInt(1, y);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				id = rs.getInt("id_anneAcad");
			}
			ps.close();
			conn.close();
		} catch (Exception e) {
	...
	public int anneUniv(int id) {
		Connection conn = SingletonConnection.getConnection();
		PreparedStatement ps;
		int anne = 0;
		try {
			ps = conn.prepareStatement("select anneAcad from anneacad where id_anneAcad=?");
			ps.setInt(1, id);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				anne = rs.getInt("anneAcad");
			}
			ps.close();
			conn.close();
		} catch (Exception e) {
	...
	public List<anneAcad> getLISTEAA(){
		Connection conn =SingletonConnection.getConnection();
		PreparedStatement ps;
		List<anneAcad> aa = new ArrayList<anneAcad>();
		try {
			ps=conn.prepareStatement("select * from anneacad");
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				anneAcad an = new anneAcad(rs.getInt("id_anneAcad"), rs.getInt("anneAcad"));
				aa.add(an);
			}
		} catch (Exception e) {
			// TODO: handle exception
	...
	public Module INFO_MODULE(String module) {
		Connection conn = SingletonConnection.getConnection();
		PreparedStatement ps;
		Module m = new Module();
		try {
			ps=conn.prepareStatement("select * from module where libelle_module=? ");
			ps.setString(1, module);
			ResultSet rs =ps.executeQuery();
			while(rs.next()) {
				m.setIDModule(rs.getInt("id_module"));
				m.setCoeff(rs.getInt("coefficient"));
				m.setLabelleMod(rs.getString("libelle_module"));
				m.setIDSemestre(rs.getInt("fid_semestre"));
				m.setNote_valid(rs.getInt("note_valid"));
				m.setNbr_elt(rs.getInt("nbr_elt"));
				m.setNe(rs.getInt("Note_eliminatoire"));
	...
	public anneAcad getAA(int id) {
		Connection conn = SingletonConnection.getConnection();
		PreparedStatement ps;
		anneAcad aa =new anneAcad(0, 0);
		try {
			ps=conn.prepareStatement("select * from anneacad where id_anneAcad=?");
			ps.setInt(1, id);
			ResultSet rs =ps.executeQuery();
			while(rs.next()) {
				aa.setId_anneAcad(rs.getInt("id_anneAcad"));
				aa.setAnneacad(rs.getInt("anneAcad"));
			}
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
	...
	public double getNoteElement(String massarEtud, String elt,int id_anne) {
		...
		ps=conn.prepareStatement("update note_elt set NOTE=? ,Etat=? where id_etudiantt=? and id_eltt=? and fid_ac=? ");
	...
	public double NotedsModule(String module,String massarEtud,int id_anne ) {
		...
		for (int i = 0; i < elts.size(); i++) {
			...
			double m = getNoteElement(massarEtud, e.getLabelleElement(),id_anne);
			tab[i]=m;
			c[i]=e.getCoeff();
			sc +=c[i];
		}
		...
		somme=moyenne/sc;
		double s = Math.round(somme*100.0)/100.0;//12.333333333333339~12.34
		return s ;
	}
	
	public String get_Etat_M(String module,String massarEtud,int id_anne) {
		String etat =null;
		double d = NotedsModule(module, massarEtud,id_anne);
		etat=d>=10?"Valide":"Rattrapage";
		return etat;
	}
	
	public void saveNOTE_ELT(String massarEtud , String element ,double TP,double ORD,double CC,int id_anne) {
		Connection conn = SingletonConnection.getConnection();
		PreparedStatement ps;
		int id_elt = getIDElement(element);
		try {
			ps =conn.prepareStatement("insert into note_elt(id_etudiantt,id_eltt,Note_tp,Note_ExamO,Note_CC,fid_ac) values(?,?,?,?,?,?) ");
			...
			ps.setInt(6, id_anne);
			ps.executeUpdate();
			getNoteElement(massarEtud, element,id_anne);
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
		}
	...
	public double recuperer_note(String massarEtud,String element,int id_anne) {
		Connection conn = SingletonConnection.getConnection();
		PreparedStatement ps;
		int id_elt = getIDElement(element);
		double note =0;
		try {
			ps=conn.prepareStatement("select  NOTE from note_elt where id_etudiantt=? and id_eltt=? and fid_ac=?");
			ps.setString(1, massarEtud);
			ps.setInt(2, id_elt);
			ps.setInt(3, id_anne);
			ResultSet rs =ps.executeQuery();
	...
	public void calcule_Ratt(String massarEtud,String element,double Note_ExamR,int id_anne) {
		List<ETUD_NOTE> ratt = getRattrapage(element);
		for (int i = 0; i < ratt.size(); i++) {
			if(ratt.get(i).getCNE().equals(massarEtud)) {
				break;
			}
		}
		double NOTE=Math.max(Note_ExamR, getNoteElement(massarEtud, element,id_anne));
		update_Note(NOTE, massarEtud, element);
		
				
	}

	public void update_Note(double n , String massarEtud,String element) {
		Connection conn = SingletonConnection.getConnection();
		PreparedStatement ps,ps1;
		String etat=null;
		int id_element = getIDElement(element);
		try {
			ps=conn.prepareStatement("UPDATE note_elt SET NOTE = ?,Note_ExamR=? WHERE id_etudiantt = ? and id_eltt = ?;\r\n"
					+ "");
			ps.setString(3, massarEtud);
			ps.setInt(4, id_element);
			ps.setDouble(1, n);
			ps.setDouble(2, n);
			ps.executeUpdate();
			ps.close();
			etat=n>=10?"Valide":"Non Valide" ;
			ps1=conn.prepareStatement("UPDATE note_elt SET Etat = ? WHERE id_etudiantt = ? and id_eltt = ?;");
			ps1.setString(1, etat);
			ps1.setString(2, massarEtud);
			ps1.setInt(3, id_element);
			ps1.executeUpdate();
			ps1.close();
		} catch (Exception e) {
			// TODO: handle exception
	...
	public void UpdateSemestre(Semestre semestre) {
		...
		PreparedStatement ps;
		try {
			ps=conn.prepareStatement("update semestre set libelle_semestre=?,fid_etap=?,note_validation=? where id_semestre=?");
			ps.setString(1,semestre.getLabelleSemestre());
			ps.setInt(2,semestre.getIDEtape());
			ps.setInt(3,semestre.getNoteValidation());
			ps.setInt(4, semestre.getIDSemestre());
			ps.executeUpdate();
			ps.close();
			conn.close();
	...
	public Semestre getIdSemestre(int id) {
		Connection conn=SingletonConnection.getConnection();
		PreparedStatement ps;
		Semestre s=new Semestre();
		try {
			ps=conn.prepareStatement("select * from semestre where id_semestre=? ");
			ps.setInt(1, id);
			ResultSet rs=ps.executeQuery();
			while(rs.next()) {
				s.setIDSemestre(rs.getInt("id_semestre"));
				s.setIDEtape(rs.getInt("fid_etap"));
				s.setLabelleSemestre(rs.getString("libelle_semestre"));
				s.setNoteValidation(rs.getInt("note_validation"));
			}
			ps.close();
			conn.close();
	...
	public void UpdateEtape(Etape etape) {
		...
		PreparedStatement ps;
		try {
			ps=conn.prepareStatement("update etape set libelle_etape=?,diplomante=?,fid_fil=? where id_Etape=?");
			ps.setString(1, etape.getLabelleEtape());
			int b = etape.isDiplomante()?1:0;//
			ps.setInt(2, b);
			ps.setInt(3, etape.getIDFiliere());
			ps.setInt(4, etape.getIDEtape());
			ps.executeUpdate();
	...
	public void UpdateFiliere(Filiere filiere) {
		...
		PreparedStatement ps;
		try {
			ps=conn.prepareStatement("update filiere set nom_filier=?,fid_etab=? where id_filiere=?");
			ps.setString(1, filiere.getFiliere());
			ps.setInt(2, filiere.getIDetab());
	...
	public Semestre getSemestre(int id_semestre) {
		...
		PreparedStatement ps;
		try {
			ps=con.prepareStatement("select id_semestre , libelle_semestre,fid_etap from semestre where id_semestre=?");
			ps.setInt(1, id_semestre);
			ResultSet rs =ps.executeQuery();
			while(rs.next()) {
				
				sem.setIDSemestre(rs.getInt("id_semestre"));
				sem.setLabelleSemestre(rs.getString("libelle_semestre"));
				sem.setIDEtape(rs.getInt("fid_etap"));
			}
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
	...
	public String getElement(int id_elt) {
		Connection conn = SingletonConnection.getConnection();
		PreparedStatement ps;
		String elt=null;
		try {
	...
	public List<Semestre> SEMfromFILIERE(String nomf){
		Connection conn = SingletonConnection.getConnection();
		PreparedStatement ps;
		List<Semestre> s = new ArrayList<Semestre>();
		String elt=null;
		try {
			ps=conn.prepareStatement("select s.libelle_semestre from semestre s , etape e , filiere f where f.id_filiere=e.fid_fil and f.nom_filier=? and e.id_Etape=s.fid_etap;");
			ps.setString(1, nomf);
			ResultSet rs =ps.executeQuery();
			while(rs.next()) {
				Semestre ss = new Semestre(rs.getString("libelle_semestre"));
				s.add(ss);
			}
		} catch (Exception e) {
			// TODO: handle exception
